    ...
    if ($db->num_rows() > 0) {
        ...
        while($system = $db->fetchrow($q_systems))
        {
            ...
            if ($size>2)
            {
                ...
                $map_data.='<area href="index.php?a=tactical_cartography&system_id='.encode_system_id($system['system_id']).'" target="_self" shape="rect" coords="'.$px_x.','.$px_y.', '.($px_x+$size-2).', '.($px_y+$size-2).'" title="'.$useTitle.'">';
            }
            else
            {
